...
parser.add_argument('--save_dir',type=str, default='results/sigma009/test',help="save directory")
parser.add_argument('--load_dir',type=str, default='results/sigma008',help="load model directory")
...
for epoch in range(opt.epochs):
    ...
    if training_loss_epoch < min_loss:
        ...
        save_name = os.path.join(opt.save_dir,'model_best_test')
        save_checkpoint(save_name,model,latent_vecs,optimizer)

    if (epoch+1) % opt.log_interval == 0:
        save_name = os.path.join(opt.save_dir,'model_routine_test')
        save_checkpoint(save_name,model,latent_vecs,optimizer)


save_name = os.path.join(checkpoint_dir,opt.save_dir,'model_test')
save_checkpoint(save_name,model,latent_vecs,optimizer)
